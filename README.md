...
```{figure} https://i.gyazo.com/b278626f981e262692ad3cfee1560681.gif
このようなメニューを作成します。画面幅が`Mobile`（スマートフォン）の時のハンバーガーメニューには、`SlickNav`を使用しています。
```
...
```{figure} https://i.gyazo.com/9d8fba6733cc30fe4d322dc2f5e36a31.gif
このような見た目を作ります
```

この`<!-- 1 -->`から`<!-- 3 -->`の作業は全てに従った内容となっています。
...
`SlickNav`を組み込む前の`HTML`は以下の通りとします。
...
1. <!-- 1 --> の箇所
	`SlickNav`を構成する以下の３ファイルを読み込んでいます。
		a. `jQuery`本体
		b. `SlickNav`本体
		c. `SlickNav`用のCSS
2. <!-- 2 --> の箇所
	`SlickNav` の素材となる<ul> を用意します
3. <!-- 3 --> の箇所
	閉じタグ`</body>` の直前に`SlickNav` を起動する`<script> `（`JavaScript`プログラム）を入れます
...
上記で説明した基本的な利用方法に対し、オプションの設定を追加することが可能です。
...
「MENU」のラベルを空にするように、<!-- 3 --> にラベルオプション（`label`）を追加します。
...
```{figure} https://i.gyazo.com/84bfdb310cb49e6924caf22fed7ce903.png
「MENU」の表示が消えました。
```
...
`SlickNav`はハンバーガーメニュー表示のために`HTML`に`div`を追加します。
デベロッパーツールから追加される`div`を把握することで、見た目のカスタマイズを行うことができます。

### デベロッパーツールから、追加される`div`を調べておく

```{figure} https://i.gyazo.com/d43d1d46d14a31906262be863044a761.png
デベロッパーツールから調べると`<div class="slicknav_menu">`が追加されていることがわかります。
```

同様に`<div class="slicknav_menu>`の中を開いてみると、以下のような構造になっているのがわかります。
```
...
このうち、`a`要素（`a.slicknav_btn`）はハンバーガーメニューのアイコンの部分で、`ul.slicknav_nav`がアイコンをクリックしたときにプルダウンされるナビゲーション本体になります。

これらを念頭に置いてカスタマイズ作業を行っていきます。
...
`Mobile`と`Desktop`でハンバーガーメニューと通常のナビゲーションである`ul`を切り替える`CSS`は以下コードになります。
...
```{figure} https://i.gyazo.com/780404bdbfdf1cf0d2c7a8d8df6ff91f.gif
...
```{figure} https://i.gyazo.com/4e4f7502e28eaaefd15079a958c2000f.png
この`HTML`をプレビューすると、`SlickNav`は `<body>`先頭に追加されています。
```

この`SlickNav`の位置を変更してみたいと思います。
`SlickNav`には自身を挿入する場所を指定できるオプション（`appendTo`、〜の後に追加する）がありますので、指定してみます。
...
```{figure} https://i.gyazo.com/cfc647ca5dc00aa65cefaca74bbbc5db.png
`<header>`の末尾に追加されるようになりました。
```

#### ハンバーガーメニューのアイコンだけにする

ハンバーガーメニューが横に長く伸びていますが、これをアイコンだけの表示に切り替えたいと思います。
次の`CSS`を加えます。
...
```{figure} https://i.gyazo.com/4bdf5edbc05e90ddda7a658c56403a93.png
アイコンだけになりました。
```

```{hint}
ここ以降の作業は、デベロッパーツールで何に対しどんなプロパティを加えたらいいかを実験で確かめながら行っています。
見た目をカスタマイズする作業を行うのに、`CSS`を書き換えて実験することが容易なデベロッパーツールはたいへん有用です。
デベロッパーツールを活用しましょう。
```
...
```{figure} https://i.gyazo.com/09fe10ab6ae2273216ed0784556f63cc.png
アイコンの位置が変わりました。
```

#### アイコンクリック時のプルダウンメニューの背景色を設定する

```{figure} https://i.gyazo.com/b2a94d55efe7cf832f2c3867e4018946.png
...
```{figure} https://i.gyazo.com/534ebad4630121c2740e726718774503.png
